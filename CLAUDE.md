...
# (The settings window is the app's main window — it opens on every launch and shows in the
#  taskbar. There is no tray icon; closing it, confirmed, quits the whole app.)
...
- The fuel **consumption table** is a second, independent fuel widget off the same `FuelModel`: `Data/FuelTableBuilder.Build(RawTick, FuelModel, OverlayConfig)` → `FuelTableReady` → `UI/FuelTableWindow`. Rows Last / Last-5 / Last-10 / Stint / Target (usage L/lap + laps-to-empty) plus a target-vs-actual "ahead/behind + need X/lap" tracker; works in **practice and race** (no session/confidence gating). The Target is two interlocked numbers (laps ↔ L/lap, tied by the usable tank; `TargetMode` = last one set wins), editable in settings and — when `FuelTable.Interactive` is on (default off, else stays click-through) or in edit mode — clickable via ▲/▼ on the widget. `FuelModel` grew a rolling non-pit-lap usage buffer + per-stint accumulator to feed it. Spec: `docs/FUEL-STRATEGY.md`.
...
- `UI/SettingsWindow` is the app's **main window** and its only real chrome: a normal, activatable, taskbar-present (dark-title-bar via `Win32.UseDarkTitleBar`) window shown on every launch. It is a **normal, non-topmost window** (do not make it topmost — that was tried and rejected). `BringToFront()` (called on Loaded) restores+focuses it. There is **no tray icon** — the app is a normal Windows app. **Shared-UI-thread gotcha:** every widget window renders on this same UI thread, and the standings `Render` is a full ItemsSource rebuild (with `SharedSizeGroup` → global layout) that can peg the thread in a busy race. All the widgets therefore dispatch their per-tick `Render` at `DispatcherPriority.Background` (below `Input`) so mouse/keyboard input on the settings window is never starved — keep it that way, and don't raise those repaints back to Normal priority. `ShutdownMode=OnMainWindowClose` + `App.MainWindow = _settings`, so closing this window quits everything; `SettingsWindow.OnClosing` shows a confirmation prompt first (the overlays go with it), and the footer "Quit overlay" button routes through the same `Close()`. Minimize to get it out of the way without quitting. Connection status (old tray tooltip) shows under the brand via `SetStatus`; a newer-release notice (old tray menu item) appears as the nav-rail update banner via `ShowUpdateAvailable`. It edits `ConfigService.Current` live and persists through **`ConfigService.SaveAndNotify()`** — plain `Save()` suppresses the watcher echo, so in-app edits MUST use `SaveAndNotify` to reach the widgets; external file edits still flow via the watcher. Rows are built from descriptor helpers (`Toggle`/`Slider`/`Segmented`/`ColorRow`/`Master`), so adding a setting is one line — do not hand-roll bound XAML per control. Every widget's `Enabled` flag and the "Move overlays" edit-mode toggle route here; edit mode has one owner (`App.SetEditMode`) that mirrors state back to the settings switch. Autostart lives in `Config/AutoStart`.

## Constraints & gotchas

- **Licensing**: uses irsdkSharp (MIT). Do NOT switch to IRSDKSharper — it is GPL-3.0 and conflicts with this repo's MIT license.
- The app is pure WPF (no `UseWindowsForms`) — the tray `NotifyIcon` was the only WinForms user and is gone. Audio is `System.Media.SoundPlayer` (base library). Don't reintroduce a WinForms dependency for chrome; the settings window is the app's window.
