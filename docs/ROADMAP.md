...
- [x] **Settings header redesign + fuel split (2026-09-14)** — reworked the top of every settings
  page to: title, subtitle, a contextual **"show this box"** switch (enables the widget for the
  section you're on — hidden on non-widget pages; greys the page body when off), the global **Move
  overlays** switch, then the **In car | Spectating** state switcher, then the normal settings.
  "Move overlays" and the per-box enable are now reachable from every page (both were buried before:
  move overlays only on General, enable only as an in-body master toggle per section). The old master
  toggle is gone from each widget section — its enable lives in the header. Split the Fuel nav section
  into **Fuel** (strategy widget) and **Fuel Table** (consumption table), each its own page + reset.
