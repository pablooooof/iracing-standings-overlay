...
4. **State machine (per car), with hysteresis.**
   - HIDDEN → **WATCH** when `tta ≤ AlertLeadTimeSec` (12). Blue additionally fires on raw
     ...
     **Alert basis** (`ShowTimeToArrival`, default false = **Gap**): the *whole* state machine —
     appear, escalate, sort, and the printed number — keys off the on-track **gap** on the centre
     car's real-pace ruler (`RelativeGap.ActualLap`), so a car pops/escalates at exactly the number
     the relative box shows for it, and `AlertLeadTimeSec`/`ImminentSec` read as gap-seconds. Set
     it true for the **Countdown** basis: everything keys off time-to-arrival (`gap ÷ rate`) and the
     number is the countdown. The per-class detection gap still drives the tracking *window*, the
     closing-rate ring buffer and the blue-proximity shortcut (so multiclass windowing is stable);
     only the trigger/escalate/sort metric follows the basis. Blue and lapping rows always show the
     gap. Note the trade-off: a fast GTP appears when it reaches the gap lead (e.g. 12 s of gap),
     which is *later* than a countdown would — raise the lead time if you want more warning.
